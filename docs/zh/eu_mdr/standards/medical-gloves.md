---
title: 协调标准 — 医用手套
description: "EU MDR 2017/745 协调标准：医用手套（3条入官方公报标准），适用于GSPR 10.4。基于 CID (EU) 2021/1182 及修正案 2026/760。"
regulation: EU MDR 2017/745
category: Medical Gloves
---

# 协调标准 — 医用手套

**官方来源**：[EC Health — Harmonised Standards](https://health.ec.europa.eu/medical-devices-topics-interest/harmonised-standards_en) | 基于 CID (EU) 2021/1182（合并版）及修正案 [CID (EU) 2026/760](https://eur-lex.europa.eu/eli/dec_impl/2026/760/oj)

## 协调标准列表（3条）

| 标准号 | 标题摘要 | GSPR对应 | 状态 |
|--------|---------|---------|------|
| **EN 455-1:2020+A2:2024** | 第1部分：无孔要求和试验 | GSPR 10.4（医用手套） | 现行有效 |
| **EN 455-2:2024** | 第2部分：物理性能的要求和试验 | GSPR 10.4（医用手套） | 现行有效 |
| **EN 455-3:2023** | 第3部分：生物学评价的要求和试验 | GSPR 10.4（医用手套） | 现行有效 |

## 相关页面

- [附件I — GSPR](../regulations/annex-i-gspr)

## 数据层源文件

[eu_mdr/standards/standards-medical_gloves.json](https://github.com/qian-qiang/docmcp-knowledge/tree/main/eu_mdr/standards/standards-medical_gloves.json)
