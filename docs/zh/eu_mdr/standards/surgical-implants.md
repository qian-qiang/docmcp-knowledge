---
title: 协调标准 — 非有源外科植入物
description: "EU MDR 2017/745 协调标准：非有源外科植入物（4条入官方公报标准），适用于GSPR 10, 14。基于 CID (EU) 2021/1182 及修正案 2026/760。"
regulation: EU MDR 2017/745
category: Non-active Surgical Implants
---

# 协调标准 — 非有源外科植入物

**官方来源**：[EC Health — Harmonised Standards](https://health.ec.europa.eu/medical-devices-topics-interest/harmonised-standards_en) | 基于 CID (EU) 2021/1182（合并版）及修正案 [CID (EU) 2026/760](https://eur-lex.europa.eu/eli/dec_impl/2026/760/oj)

## 协调标准列表（4条）

| 标准号 | 标题摘要 | GSPR对应 | 状态 |
|--------|---------|---------|------|
| **EN ISO 7197:2024** | 无菌、一次性使用的脑积水分流器及部件 | GSPR 10, 14（非有源外科植入物） | 现行有效 |
| **EN ISO 14630:2024** | 通用要求 | GSPR 10, 14（非有源外科植入物） | 现行有效 |
| **EN ISO 21535:2024** | 关节置换植入物 - 髋关节置换植入物的特殊要求 | GSPR 10, 14（非有源外科植入物） | 现行有效 |
| **EN ISO 21536:2024** | 关节置换植入物 - 膝关节置换植入物的特殊要求 | GSPR 10, 14（非有源外科植入物） | 现行有效 |

## 相关页面

- [附件I — GSPR](../regulations/annex-i-gspr)

## 数据层源文件

[eu_mdr/standards/standards-surgical_implants.json](https://github.com/qian-qiang/docmcp-knowledge/tree/main/eu_mdr/standards/standards-surgical_implants.json)
