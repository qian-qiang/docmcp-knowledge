---
title: 协调标准 — 患者搬运设备
description: "EU MDR 2017/745 协调标准：患者搬运设备（2条入官方公报标准），适用于GSPR 9, 14。基于 CID (EU) 2021/1182 及修正案 2026/760。"
regulation: EU MDR 2017/745
category: Patient Handling Equipment
---

# 协调标准 — 患者搬运设备

**官方来源**：[EC Health — Harmonised Standards](https://health.ec.europa.eu/medical-devices-topics-interest/harmonised-standards_en) | 基于 CID (EU) 2021/1182（合并版）及修正案 [CID (EU) 2026/760](https://eur-lex.europa.eu/eli/dec_impl/2026/760/oj)

## 协调标准列表（2条）

| 标准号 | 标题摘要 | GSPR对应 | 状态 |
|--------|---------|---------|------|
| **EN 1865-2:2024** | 第2部分：电动担架 | GSPR 9, 14（患者搬运） | 现行有效 |
| **EN 1865-6:2024** | 第6部分：电动座椅 | GSPR 9, 14（患者搬运） | 现行有效 |

## 相关页面

- [附件I — GSPR](../regulations/annex-i-gspr)

## 数据层源文件

[eu_mdr/standards/standards-patient_handling.json](https://github.com/qian-qiang/docmcp-knowledge/tree/main/eu_mdr/standards/standards-patient_handling.json)
